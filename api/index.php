...
$query = <<<EOD
	...
	AND wp_term_taxonomy.term_id = 4
	ORDER BY post_date DESC
       
EOD;
    //initialize query parameters - in this case all projects being pulled thus empty array
    $params = array();
...
// get job by id
$app->get('/project/{id}', function(Request $request, $id) use ($app) {
    $query = <<<EOD
...
// get project meta by id
$app->get('/project_meta/{id}', function(Request $request, $id) use ($app) {
    $query = <<<EOD
    
    SELECT * 
	FROM wp_postmeta
	WHERE post_id = :id
	AND meta_key =  'website_url'
	LIMIT 0 , 30
